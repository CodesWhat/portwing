...
On a rejected signature, the agent returns `401` with an `X-Portwing-Reason`
response header set to one of `timestamp-skew`, `replay`, `unknown-key`, or
`invalid-signature` (the catch-all for a missing header, malformed nonce,
malformed signature, or a signature that fails verification).
