...
| `portwing_host_disk_metrics_available` | gauge | Whether host disk usage could be read from the Docker data root (`1`) or not (`0`). Independent of `portwing_host_metrics_supported` — disk uses `statfs`, not `/proc`. When `0`, the disk byte series below are omitted rather than written as zeros. |
...
<Callout type="info">
Disk metrics (`portwing_host_disk_total_bytes`, `portwing_host_disk_used_bytes`) reflect the filesystem that contains the Docker data root, resolved from the daemon's `/info` rather than assumed to be `/var/lib/docker`. They are omitted both when the agent is started with disk collection disabled and when the `statfs` read itself fails (`portwing_host_disk_metrics_available 0`).
</Callout>
...
portwing_build_info{version="0.9.11"} 1
...
```json
{"status":"ok","live":true,"ready":false,"mode":"standard","version":"0.9.11","uptimeSeconds":3842.7,"docker":"unknown","controller":"not_applicable"}
```
...
```json
{"status":"healthy","live":true,"ready":true,"mode":"standard","version":"0.9.11","uptimeSeconds":3842.7,"docker":"connected","controller":"not_applicable"}
```

Edge-mode not-ready response:

```json
{"status":"unhealthy","live":true,"ready":false,"mode":"edge","version":"0.9.11","uptimeSeconds":42.1,"docker":"connected","controller":"disconnected"}
```
