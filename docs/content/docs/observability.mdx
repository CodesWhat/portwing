...
| `portwing_auth_failures_total` | counter | `reason` | Total authentication failures, labelled by failure reason (e.g. `bad-token`, `missing-token`). |
...
portwing_auth_failures_total{reason="bad-token"} 3
```
...
portwing_build_info{version="0.3.0"} 1
