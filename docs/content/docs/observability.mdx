...
local operations listener exposes only `/health`, `/ready`,
`/_portwing/health`, and `/metrics`; keep that listener on a private container
or cluster network because the metrics route has no inbound credential surface.
...
| `portwing_audit_exports_total{outcome}` | counter | standard | JSON/NDJSON export requests split into `success` and `error`. |
...
portwing_build_info{version="0.8.0"} 1
...
```json
{"status":"ok","live":true,"ready":false,"mode":"standard","version":"0.8.0","uptimeSeconds":3842.7,"docker":"unknown","controller":"not_applicable"}
```
...
```json
{"status":"healthy","live":true,"ready":true,"mode":"standard","version":"0.8.0","uptimeSeconds":3842.7,"docker":"connected","controller":"not_applicable"}
```

Edge-mode not-ready response:

```json
{"status":"unhealthy","live":true,"ready":false,"mode":"edge","version":"0.8.0","uptimeSeconds":42.1,"docker":"connected","controller":"disconnected"}
```
