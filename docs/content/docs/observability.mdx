...
portwing_build_info{version="0.9.19"} 1
...
```json
{"status":"ok","live":true,"ready":false,"mode":"standard","version":"0.9.19","uptimeSeconds":3842.7,"docker":"unknown","controller":"not_applicable"}
```
...
```json
{"status":"healthy","live":true,"ready":true,"mode":"standard","version":"0.9.19","uptimeSeconds":3842.7,"docker":"connected","controller":"not_applicable"}
```

Edge-mode not-ready response:

```json
{"status":"unhealthy","live":true,"ready":false,"mode":"edge","version":"0.9.19","uptimeSeconds":42.1,"docker":"connected","controller":"disconnected"}
```
...
selects HTTP or HTTPS from `TLS_CERT`. HTTPS probes require an exact match to
the configured leaf certificate, so self-signed certificates and certificates
without a `localhost` name still work. The Kubernetes examples use `/health`
for liveness and `/ready` for readiness.
