...
All Edge Mode wire messages are wrapped in an `Envelope` (`{"type": ..., "data": ...}`, `internal/protocol/messages.go`) — the fields above live under `data`, not at the top level.
...
| `dd:container_delete_response` | Agent → Controller | Removal result (`success`/`error`) |
