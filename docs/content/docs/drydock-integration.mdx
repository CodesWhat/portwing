...
<Callout type="info">
`memoryGb` is `0` on Portwing 0.2.0 and earlier — the current release keeps the binary cgo-free and does not read host memory. Drydock accepts `0` as valid. A later release reports real memory on Linux by reading `/proc/meminfo` (still no cgo).
</Callout>
