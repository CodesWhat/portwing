...
**Edge Mode** (stable `portwing/1.0`; requires Drydock 1.5+): Portwing dials outbound to the Drydock controller's `/api/portwing/ws` WebSocket endpoint. All communication is multiplexed over the single connection. This works behind NAT and dynamic IPs.
