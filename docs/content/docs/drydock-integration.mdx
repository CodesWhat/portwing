...
**Edge Mode** (implemented end-to-end; requires Drydock 1.5+ and Portwing 0.2.2+): Portwing dials outbound to the Drydock controller's `/api/portwing/ws` WebSocket endpoint. All communication is multiplexed over the single connection. This works behind NAT and dynamic IPs.

<Callout type="warn">
Edge Mode is implemented on both sides: Drydock 1.5 ships the `/api/portwing/ws` controller endpoint (Ed25519-only, `portwing/1.0` protocol) and Portwing's agent (`internal/edge/client.go`) signs its hello with Ed25519. Drydock 1.5 and Portwing 0.2.2 are both pre-release — the current stable lines (Drydock 1.4.x, Portwing 0.2.0) do **not** include the endpoint — so treat Edge Mode as early access and prefer Standard Mode for production until the stable releases ship.
</Callout>
...
<Callout type="info">
`memoryGb` is `0` on Portwing 0.2.1 and earlier — the current release keeps the binary cgo-free and does not read host memory. Drydock accepts `0` as valid. A later release reports real memory on Linux by reading `/proc/meminfo` (still no cgo).
</Callout>
...
When `DRYDOCK_URL` is set, Portwing dials outbound to `DRYDOCK_URL/api/portwing/ws` using the `portwing/1.0` WebSocket protocol.
...
| Edge Mode WebSocket `/api/portwing/ws` | Implemented (Drydock 1.5) | Ed25519-only, `portwing/1.0`; requires Drydock 1.5+ and Portwing 0.2.2+ (both pre-release) |
