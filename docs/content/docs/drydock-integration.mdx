...
**Edge Mode** (stable `portwing/1.0`; enabled by default in Drydock 1.6): Portwing dials outbound to the Drydock controller's `/api/portwing/ws` WebSocket endpoint. All communication is multiplexed over the single connection. This works behind NAT and dynamic IPs.

<Callout type="info">
Edge Mode is implemented on both sides and is covered by a real multi-agent reconnect/load soak. Drydock 1.6 enables its Ed25519-only endpoint by default; `DD_EXPERIMENTAL_PORTWING=false` is retained as an emergency disable. Drydock 1.5 deployments must explicitly set `DD_EXPERIMENTAL_PORTWING=true`.
</Callout>
...
Verified against Drydock 1.6.x (`app/agent/AgentClient.ts`).
...
<Callout type="info">
The sequence below is the production handshake, implemented on the agent side in `internal/edge/client.go` and `internal/protocol/messages.go` and on the controller side in Drydock 1.6's `/api/portwing/ws` endpoint. The controller requires an Ed25519-signed hello.
</Callout>

<Callout type="info">
Drydock 1.6 enables the endpoint by default. `DD_EXPERIMENTAL_PORTWING=false` is an emergency disable. Drydock 1.5 deployments must set `DD_EXPERIMENTAL_PORTWING=true`.
</Callout>
...
Verified against Drydock 1.6.x (`app/agent/AgentClient.ts`, `app/agent/api/`).
...
| Edge Mode WebSocket `/api/portwing/ws` | Stable (`portwing/1.0`) | Ed25519-only; Drydock 1.6 enables it by default. Continuous logs and the real fleet-soak gate require Portwing 0.8+. |
