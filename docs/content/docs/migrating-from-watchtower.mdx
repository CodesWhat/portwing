...
<Callout type="info">
Edge mode is production supported under the stable `portwing/1.0` protocol and is covered by a real multi-agent reconnect/load soak. Drydock 1.6 enables the endpoint by default; Drydock 1.5 requires `DD_EXPERIMENTAL_PORTWING=true`. The endpoint is Ed25519-only: set `PRIVATE_KEY_FILE` (token-only connections are rejected).
</Callout>
