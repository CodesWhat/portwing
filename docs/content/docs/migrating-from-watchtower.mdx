...
<Callout type="warn">
Edge mode is implemented end-to-end: Drydock 1.5 ships the `/api/portwing/ws` controller endpoint and Portwing 0.3.0's agent signs an Ed25519 hello. It's still new — exec robustness under sustained load is not yet battle-tested — so treat edge mode as early access and prefer standard mode for production until you've validated it for your workload. The endpoint is Ed25519-only: set `PRIVATE_KEY_FILE` (token-only connections are rejected).
</Callout>
