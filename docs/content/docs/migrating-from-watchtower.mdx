...
description: Replace an archived Watchtower installation with Drydock + Portwing — including architecture differences, label mapping, and a step-by-step cutover.
---
...
<Callout type="warn">
Edge mode is implemented end-to-end: Drydock 1.5 ships the `/api/portwing/ws` controller endpoint and Portwing's agent signs an Ed25519 hello. Drydock 1.5 and Portwing 0.2.2 are both pre-release (current stable: Drydock 1.4.x, Portwing 0.2.0), and full exec robustness under load lands in Portwing 0.2.2 — treat edge mode as early access and prefer standard mode for production until the stable releases ship. The endpoint is Ed25519-only: set `PRIVATE_KEY_FILE` (token-only connections are rejected).
</Callout>
...
Edge mode requires `DRYDOCK_URL` and, for the Drydock 1.5 controller, an Ed25519 key via `PRIVATE_KEY_FILE` — the endpoint rejects token-only connections. If `DRYDOCK_URL` is missing, Portwing falls back to Standard mode.
...
2. **Deploy Portwing** using the Standard or Edge compose snippet above. See [Getting Started](/getting-started) for the hardened sockguard configuration.
...
4. **Add Portwing to Drydock** — configure the agent endpoint in the Drydock UI, or let Edge mode auto-register.

5. **Verify** — check that `/_portwing/health` returns `{"status":"healthy"}` and the Drydock UI shows the host and its containers.
...
<Callout type="info">
See [Configuration](/configuration) for the full environment-variable reference and [Security Model](/security-model) for Portwing's default-deny socket filtering and rate-limiting details.
</Callout>
