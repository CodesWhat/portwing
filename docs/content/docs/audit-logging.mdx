...
<Callout type="warn">
  The audit event schema is marked stable in `internal/audit/audit.go`, but Portwing is currently in v0.5.x alpha. Field additions are non-breaking; field renames or removals will be announced in the changelog before they ship.
</Callout>
