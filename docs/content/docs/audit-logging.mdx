...
<Callout type="warn">
  The audit event schema is marked stable in `internal/audit/audit.go`, but Portwing is currently in v0.7.x alpha. Field additions are non-breaking; field renames or removals will be announced in the changelog before they ship.
</Callout>
...
{
  "records": [
    {"ts":"2026-01-15T10:24:01.500000000Z","event":"exec_start","actor":"203.0.113.42","container":"abc123def456","exec_id":"e7f8a9b1","outcome":"allowed"},
    {"ts":"2026-01-15T10:23:50.001000000Z","event":"auth_failure","actor":"203.0.113.9","method":"GET","path":"/_portwing/info","outcome":"denied"}
  ],
  "count": 2
}
```

The ring buffer's JSON schema differs from the streamed slog log schema documented above — records are JSON-encoded directly from the internal `audit.Record` struct, which uses a `ts` field and has no `level` or `msg` field.
