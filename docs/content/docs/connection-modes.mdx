...
**Status: production supported with stable `portwing/1.0` — enabled by default in Drydock 1.6.**

<Callout type="info">
  Edge mode is covered by a real multi-agent reconnect/load soak, including concurrent
  exec, continuous logs, and controller backpressure. Drydock 1.6 enables the endpoint
  by default; Drydock 1.5 requires `DD_EXPERIMENTAL_PORTWING=true`. The endpoint is
...
| Controller-side support | Complete | Drydock 1.6 (1.5 opt-in) |
| Recommended for production | Yes | Yes (Drydock 1.6; Portwing 0.8+) |
