...
description: Run Lookout as a hardened remote Docker API without a Drydock platform connection, using the generic REST adapter.
---

Lookout normally operates in one of two connection modes — edge (WebSocket to Drydock) or standard (Drydock-compatible HTTP server). **Standalone mode** is a third option: set `ADAPTER=generic` and Lookout becomes a self-contained, security-hardened remote Docker API with no external controller required.
...
<Callout type="warn">
Never expose Lookout directly on a public interface without TLS. Run it behind a reverse proxy that terminates TLS, or supply `TLS_CERT` and `TLS_KEY` to enable the built-in TLS listener. See [Configuration](/configuration).
</Callout>
...
| `GET /_lookout/health` | Returns Docker connectivity status — HTTP 503 if daemon is unreachable |
...
curl -sf http://localhost:3000/_lookout/health
```
...
`GET /_lookout/info` returns agent version, Docker version, connection mode, uptime, hostname, and the capability list. Auth required.
...
| `X-Lookout-Token: TOKEN` | Alternate header |
| `X-Lookout-Signature` + three companion headers | Ed25519 per-request signature |
...
Lookout also exposes a read-only [Model Context Protocol](https://modelcontextprotocol.io/) server at `POST /_lookout/mcp`. It is available in all adapter modes, including `generic`. AI assistants (Claude, Cursor, Windsurf) can query live container state through this endpoint without issuing raw REST calls.
