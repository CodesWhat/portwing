...
# sockguard.yaml.
#
# The bundled sockguard.yaml is a copy of sockguard's own portwing.yaml preset:
# container lifecycle, image pull/inspect/remove, events, and narrow network/
# volume reads. Exec is denied by this preset — swap in sockguard's
...
<Callout type="info">
If you manage Docker Compose stacks through Portwing, the bundled `sockguard.yaml` is a copy of sockguard's `portwing.yaml` preset (container lifecycle, image ops, events, narrow network/volume reads). Compose deploys require additional rules — extend `sockguard.yaml` deliberately (network create, volume create, build) rather than bypassing the filter. That preset also denies exec: `POST /containers/*/exec` matches no allow rule and returns 403. For interactive exec, swap in sockguard's `portwing-with-exec.yaml`, shipped here as [`examples/sockguard-with-exec.yaml`](https://github.com/CodesWhat/portwing/blob/main/examples/sockguard-with-exec.yaml); for Drydock's edge exec feature use [`examples/docker-compose.edge-with-exec.yml`](https://github.com/CodesWhat/portwing/blob/main/examples/docker-compose.edge-with-exec.yml), which pairs edge mode with that preset.
</Callout>
