...
<Callout type="info">
**Production supported with Portwing 0.8+ and Drydock 1.6.** The stable `portwing/1.0` path is enabled by default in Drydock 1.6 and covered by a real multi-agent reconnect/load soak with concurrent exec, continuous logs, and controller backpressure. Drydock 1.5 remains an opt-in compatibility path via `DD_EXPERIMENTAL_PORTWING=true`. The endpoint is **Ed25519-only**: set `PRIVATE_KEY_FILE` and register the public key with Drydock; token-only connections are rejected.
</Callout>
