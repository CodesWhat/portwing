...
<Callout type="warn">
**Early access — requires Drydock 1.5 and Portwing 0.2.2.** The `/api/portwing/ws` controller endpoint shipped in Drydock 1.5 and Portwing's agent signs its hello with Ed25519, so edge mode now works end-to-end. Both releases are still pre-release (the current stable lines are Drydock 1.4.x and Portwing 0.2.0), and full exec-session robustness under load lands in Portwing 0.2.2 — prefer standard mode for production until the stable releases ship. The endpoint is **Ed25519-only**: set `PRIVATE_KEY_FILE` and register the public key with Drydock; token-only connections are rejected.
</Callout>
...
#   portwing keygen -comment "edge-host-01" > portwing_ed25519.pem
# Register the printed public key (POST /api/v1/portwing/keys).
