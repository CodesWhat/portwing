---
title: MCP Server
description: Lookout's built-in read-only Model Context Protocol endpoint lets MCP-aware AI agents query live Docker container and host state.
---

Lookout ships a built-in [Model Context Protocol](https://modelcontextprotocol.io/) server at `POST /_lookout/mcp`. AI assistants — Claude Desktop, Cursor, Windsurf, or any MCP-compatible client — can call its tools to inspect containers and host metrics using their standard tool-call flow. No extra process or sidecar is needed; the endpoint is part of the main Lookout HTTP server.

<Callout type="warn">
The MCP server is available in Lookout v0.2.x (currently alpha). The tool surface is stable, but the endpoint should be treated as experimental until v1.0.
</Callout>
...
Every POST returns one JSON-RPC 2.0 response object. `GET /_lookout/mcp` returns `405 Method Not Allowed` to signal that no SSE stream is offered at this endpoint.

Authentication follows the same bearer-token scheme used by all other Lookout endpoints. See [Authentication](/authentication).

---

## Enabling the endpoint

The MCP endpoint is always active when Lookout is running — it is not gated by a separate flag or environment variable. Configure access using the standard token variables (`TOKEN`, `TOKEN_FILE`, or `TOKEN_HASH`). See [Configuration](/configuration) for the full variable reference.

```bash
# Minimal example: run Lookout with a token and connect an MCP client
docker run -e TOKEN=mysecrettoken -p 3000:3000 ghcr.io/codeswhat/lookout:latest
```
...
**Response:** a metrics snapshot including CPU, memory, disk, network I/O, and uptime. The exact shape matches the Prometheus collector used internally by Lookout.
...
Replace `YOUR_LOOKOUT_TOKEN` with the value you set in `TOKEN`, `TOKEN_FILE`, or `TOKEN_HASH` (see [Configuration](/configuration)).
