...
description: Portwing's layered defenses — from socket-level filtering and cryptographic auth to hardened runtime and tamper-evident audit logging.
---

<Callout type="warn">
Portwing is in **alpha** (v0.2.x). The controls documented here are implemented and tested, but the API surface, configuration keys, and deployment model may change before a stable release. Do not rely on this page as a compliance artifact for production systems until v1.0.
</Callout>
...
This architecture structurally prevents the class of Docker AuthZ plugin bypass vulnerabilities (CVE-2024-41110, CVE-2026-34040, CVE-2026-23944, CVE-2026-44848) where authentication is delegated to a plugin that can be bypassed by crafted requests. Portwing does not use Docker's AuthZ plugin mechanism at all.
...
| Non-root user | The Portwing binary runs as a non-root UID inside the container |
...
When `AUTHORIZED_KEYS` is configured, each client has a distinct Ed25519 keypair. Every authenticated request carries four headers (`X-Portwing-Key-ID`, `X-Portwing-Timestamp`, `X-Portwing-Nonce`, `X-Portwing-Signature`). The agent verifies the Ed25519 signature over a canonical string of `METHOD\nPATH\nbody-sha256-hex\ntimestamp\nnonce` using `crypto/ed25519.Verify` from the Go standard library.
...
At runtime Portwing compares the incoming token against the stored hash using Argon2id verification, still wrapped with `crypto/subtle` for the final byte comparison to prevent timing leakage. A compromised environment variable dump or configuration file reveals only the hash.
...
| CVE-2026-34040 | Docker AuthZ plugin bypass via oversized body silently dropped before the plugin sees it (CVSS 8.8) | Control 1 — proxy-first | Same as CVE-2024-41110; the body-stripping behaviour in Docker's plugin dispatch path is irrelevant because Portwing's auth decision is enforced upstream of it |
| CVE-2026-23944 | Arcane Docker Manager: proxy forwards to remote agent before verifying the original request is authenticated (CVSS 9.8) | Control 1 — proxy-first | Portwing's auth middleware wraps the handler, not the other way around; it cannot be bypassed by ordering |
| CVE-2026-44848 | Portainer: `/plugins/*` paths absent from the proxy authorization map, allowing non-admin users to install arbitrary Docker plugins (CVSS 9.4) | Control 1 — proxy-first | Portwing's catch-all handler authenticates all paths including `/plugins/*`; there is no per-path allowlist to misonfigure by omission |
