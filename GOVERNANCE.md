...
Portwing uses a lead-maintainer model. Design and implementation work is
proposed in public issues and pull requests. The lead maintainer owns project
scope, compatibility policy, release readiness, and final decisions when
...
| Code owners | Accounts listed in [`.github/CODEOWNERS`](.github/CODEOWNERS) | Review owned areas and identify security, protocol, and compatibility risks |
...
Routine changes are decided through pull-request review. Protocol, security,
dependency, and compatibility changes should begin with an issue that records
the problem, alternatives, and migration impact. The project seeks rough
consensus, but the lead maintainer makes the final decision when a timely choice
...
Portwing is owned by the CodesWhat GitHub organization, which has two owner
...
Project forking remains available under AGPL-3.0, but it is not the primary
continuity plan.
